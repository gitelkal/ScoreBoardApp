...
export class ScoreboardTaskViewComponent extends ScoreboardBaseComponent {
  ...
  private subscribeToTaskUpdates() {
    this.signalRService.taskCompletionUpdates.subscribe(data => {
      if (data && data.scoreboardId === Number(this.scoreboardID)) {
        if (!this.completedTasksMap.has(data.scoreboardId)) {
            this.completedTasksMap.set(data.scoreboardId, new Map());
        }
        const teamTasks = this.completedTasksMap.get(data.scoreboardId)!;
        teamTasks.set(data.teamId, (teamTasks.get(data.teamId) || 0) + 1);
      }
    });
  }
